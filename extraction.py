...
for ffile in os.listdir('legitimatefiles'):
  print(ffile)
  try:
    res = extract_infos(os.path.join('legitimatefiles/', ffile))
    res.append(1)
    ff.write(csv_delimiter.join(map(lambda x:str(x), res)) + "\n")
  except pefile.PEFormatError:
    print('\t -> Bad PE format')
for ffile in os.listdir('virusfiles'):
  print(ffile)
  try:
    res = extract_infos(os.path.join('virusfiles/', ffile))
    res.append(0)
    ff.write(csv_delimiter.join(map(lambda x:str(x), res)) + "\n")
    shutil.copy("virusfiles/"+ffile, "dst")
  except pefile.PEFormatError:
     print('\t -> Bad PE format')
